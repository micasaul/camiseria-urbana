...
export default function ProductosMismaMarca({ marcaId, productoActualId }) {
  ...
  useEffect(() => {
    ...
    const marcaUrl = `${import.meta.env.VITE_BACKEND_URL}/api/marcas/${marcaId}?populate[productos][populate][0]=variacions&populate[productos][populate][1]=variacions.imagen&populate[productos][populate][2]=promo_productos&populate[productos][populate][3]=promo_productos.promo`
    
    fetch(marcaUrl)
      .then(async res => {
        if (!res.ok) {
          return fetch(`${import.meta.env.VITE_BACKEND_URL}/api/productos?populate[0]=variacions&populate[1]=variacions.imagen&populate[2]=marca&populate[3]=promo_productos&populate[4]=promo_productos.promo&filters[$or][0][inactivo][$eq]=false&filters[$or][1][inactivo][$null]=true&pagination[pageSize]=1000`)
            .then(res2 => res2.json())
